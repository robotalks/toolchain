...
cd $OUTDIR
docker build -t robotalks-pack-$PACK-$ARCH:build --build-arg ARCH=$ARCH -f $PACK.Dockerfile .
mkdir -p $OUTDIR/docker
docker save robotalks-pack-$PACK-$ARCH:build | gzip -9 >docker/$PACK-$ARCH.tar.gz
